    ...
    fetch("../../assets/data/restaurants.json?v=" + Date.now()) // Ajustando caminho do JSON
        ...
        .then(data => {
            ...
            if (restaurant) {
                ...
                let info = `
                    <p><strong>Address:</strong> <a href="${restaurant.mapsLink}" target="_blank" class="text-decoration-none text-primary">
                        ${restaurant.address} <i class="fas fa-map-marker-alt"></i>
                    </a></p>
                    <p><strong>Opening hours:</strong> ${restaurant.hours}</p>
                    <p><strong>Available services:</strong> ${restaurant.services ? restaurant.services.join(", ") : "Não informado"}</p>
