    ...
    fetch("../../assets/data/pharmacies.json?v=" + Date.now())
        ...
        .then(data => {
            ...
            if (pharmacy) {
                ...
                let info = `
                    <p><strong>Address:</strong> <a href="${pharmacy.mapsLink}" target="_blank" class="text-decoration-none text-primary">
                        ${pharmacy.address} <i class="fas fa-map-marker-alt"></i>
                    </a></p>
                    <p><strong>Opening hours:</strong> ${pharmacy.hours}</p>
                    <p><strong>Available services:</strong> ${pharmacy.services ? pharmacy.services.join(", ") : "Não informado"}</p>
