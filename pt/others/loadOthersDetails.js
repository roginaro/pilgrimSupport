    ...
    fetch("../../assets/data/others.json?v=" + Date.now())
        ...
        .then(data => {
            ...
            if (business) {
                ...
                let info = `
                    <p><strong>Endereço:</strong> <a href="${business.mapsLink}" target="_blank" class="text-decoration-none text-primary">
                        ${business.address} <i class="fas fa-map-marker-alt"></i>
                    </a></p>
                    <p><strong>Horário de funcionamento:</strong> ${business.hours}</p>
                    <p><strong>Serviços disponíveis:</strong> ${business.services ? business.services.join(", ") : "Não informado"}</p>
